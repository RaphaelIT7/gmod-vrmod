...
function VRUtilOpenHeightMenu()
	...
	hook.Add( "PreDrawTranslucentRenderables", "vrmodheightmirror", function(depth, skybox) 
		...
		--local eyeHeightPreviewPos = Vector(g_VR.tracking.hmd.pos.x, g_VR.tracking.hmd.pos.y, g_VR.origin.z + (convarValues.vrmod_charactereyeheight + tmpEyeHeightOffset)) + Angle(0,g_VR.tracking.hmd.ang.y,0):Forward()*convarValues.vrmod_characterheadtohmddist
		...
		cam.Start({x = 0, y = 0, w = 2048, h = 2048, type = "3D", fov = g_VR.view.fov, aspect = -g_VR.view.aspectratio, origin = camPos, angles = camAng})
			render.PushRenderTarget(rt_mirror)
				...
				render.CullMode(1)
					...
					--render.DrawBox(eyeHeightPreviewPos, Angle(), Vector(-1,-1,-1),Vector(1,1,1),Color(255,0,0,255))
	...
	buttons = {
		...
		{x=250,y=200,w=50,h=50,text="+",font="Trebuchet24",text_x=25,text_y=15,enabled=not convarValues.vrmod_seated,fn=function()
			...
			--tmpEyeHeightOffset = tmpEyeHeightOffset + 0.1
			convars.vrmod_charactereyeheight:SetFloat(convarValues.vrmod_charactereyeheight + 0.2)
		...
		--[[{x=250,y=255,w=50,h=50,text="Auto\nScale",font="Trebuchet24",text_x=25,text_y=0,enabled=not convarValues.vrmod_seated,fn=function()
			convars.vrmod_charactereyeheight:SetFloat(convarValues.vrmod_charactereyeheight + tmpEyeHeightOffset)
			tmpEyeHeightOffset = 0
			g_VR.scale = convarValues.vrmod_charactereyeheight / ((g_VR.tracking.hmd.pos.z-g_VR.origin.z)/g_VR.scale)
			convars.vrmod_scale:SetFloat(g_VR.scale)
		end},]]
		{x=250,y=255,w=50,h=50,text="-",font="Trebuchet24",text_x=25,text_y=15,enabled=not convarValues.vrmod_seated,fn=function() --y 310
			--g_VR.scale = g_VR.scale - 0.5
			--convars.vrmod_scale:SetFloat(g_VR.scale)
			--tmpEyeHeightOffset = tmpEyeHeightOffset - 0.1
			convars.vrmod_charactereyeheight:SetFloat(convarValues.vrmod_charactereyeheight - 0.2)
	...
	renderControls = function()
		...
		draw.DrawText( "note: you must disable seated mode\nand stand up irl when adjusting height", "Trebuchet18", 3, -2, Color( 0, 0, 0, 255 ), TEXT_ALIGN_LEFT)
