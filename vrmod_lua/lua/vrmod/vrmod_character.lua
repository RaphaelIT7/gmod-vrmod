if CLIENT then
	...
	vrmod.AddCallbackedConvar("vrmod_charactereyeheight", nil, "66.8", nil, nil, nil, nil, tonumber, function(val)
		net.Start("vrmod_characterinfo")
		net.WriteFloat(val)
		net.WriteFloat(convarValues.vrmod_characterheadtohmddist)
		net.SendToServer()
	end)

	vrmod.AddCallbackedConvar("vrmod_characterheadtohmddist", nil, "6.3", nil, nil, nil, nil, tonumber, function(val)
		net.Start("vrmod_characterinfo")
		net.WriteFloat(convarValues.vrmod_charactereyeheight)
		net.WriteFloat(val)
		net.SendToServer()
	end)

	net.Receive("vrmod_characterinfo",function()
		local asd = g_VR.net[net.ReadString()]
		if asd == nil then return end
		asd.characterEyeHeight = net.ReadFloat()
		asd.characterHeadToHmdDist = net.ReadFloat()
	end)
	...
	local function UpdateIK(ply)
		...
		if not inVehicle then
			...
			local cutAmount = math.Clamp(charinfo.preRenderPos.z+net.characterEyeHeight - headHeight,0,40)
			--spine
			local spineLen = net.characterEyeHeight - charinfo.spineZ
			local spineTargetLen = spineLen - cutAmount*0.5
			local a1 = math.acos(spineTargetLen/spineLen)
		...
		--
		local L_UpperarmPos, _ = LocalToWorld(boneinfo[bones.b_leftUpperarm].relativePos, boneinfo[bones.b_leftUpperarm].relativeAng, L_ClaviclePos, L_ClavicleTargetAng)
		...
		--
		local R_UpperarmPos, _ = LocalToWorld(boneinfo[bones.b_rightUpperarm].relativePos, boneinfo[bones.b_rightUpperarm].relativeAng, R_ClaviclePos, R_ClavicleTargetAng)
	...
	local function CharacterInit(ply)
		...
		characterInfo[steamid] = {
			...
			--characterHeadToHmdDist = 0, --moved these 2 to net table
	...
	local function PrePlayerDrawFunc( ply )
		...
		if not ply:InVehicle() then
			characterInfo[steamid].renderPos = g_VR.net[steamid].lerpedFrame.hmdPos + up:Cross(g_VR.net[steamid].lerpedFrame.hmdAng:Right())*-g_VR.net[steamid].characterHeadToHmdDist + Angle(0,g_VR.net[steamid].lerpedFrame.characterYaw,0):Forward()*-characterInfo[steamid].horizontalCrouchOffset*0.8
...
elseif SERVER then
	util.AddNetworkString("vrmod_characterinfo")

	vrmod.NetReceiveLimited("vrmod_characterinfo",10,64,function(len, ply)
		net.Start("vrmod_characterinfo")
