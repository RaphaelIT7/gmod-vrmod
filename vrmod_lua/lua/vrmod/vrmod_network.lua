...
if CLIENT then
	...
	net.Receive("vrutil_net_join",function(len)
		...
		g_VR.net[ply:SteamID()] = {
			...
			characterEyeHeight = net.ReadFloat(),
			characterHeadToHmdDist = net.ReadFloat(),
			frames = {},
			latestFrameIndex = 0,
			buffering = true,
...
elseif SERVER then
	...
	vrmod.NetReceiveLimited("vrutil_net_join",5,66,function(len, ply)
		if g_VR[ply:SteamID()] ~= nil then 
			return 
		end
		...
		net.WriteFloat(g_VR[ply:SteamID()].characterEyeHeight)
		net.WriteFloat(g_VR[ply:SteamID()].characterHeadToHmdDist)
	...
	vrmod.NetReceiveLimited("vrutil_net_requestvrplayers",5,0,function(len, ply)
		...
		for k,v in pairs(g_VR) do
			...
			if IsValid(vrPly) then
				...
				net.WriteFloat(g_VR[k].characterEyeHeight)
				net.WriteFloat(g_VR[k].characterHeadToHmdDist)
				net.Send(ply)
			else
